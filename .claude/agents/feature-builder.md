...
- task: GitHub issue number (e.g. `#382`) — a scoped ticket (Overview/What/Scope/Acceptance Criteria) is enough; an execution-plan comment from an older `feature-planner` run is a legacy fallback, not required
...
- #398 (DB migration only) → Phase 1 only, then straight to Phase 5 (quality gates still run on migration + regenerated types) and Phase 7. No 2a/2b-middleware-excluded/2c, no Phase 3 i18n, no accessibility audit.
- #401 (middleware gate) → Phase 2b (middleware sub-case) only. No 2a, no 2c, no Phase 1, no i18n.
...
**Surface external/manual prerequisites as soon as they're identified, not saved for Phase 7.** If any phase's work needs something set up outside the codebase before it can be tested or work in production — a third-party dashboard configuration (Stripe products, an OAuth consent screen), a new env var only the user can supply, a manual account/service setup — call it out explicitly at that phase's breakpoint, with the exact steps the user needs to take. Phase 7 still compiles everything raised along the way into one final consolidated Production Checklist — it doesn't wait until then to mention any of it for the first time.
...
```bash
cd apps/academy && pnpm type-check 2>&1 | tail -5
```
...
```bash
cd apps/academy && pnpm build 2>&1 | tail -40
```
...
- backend/`lib/server` code with no JSX (e.g. #399's webhook)
- tests only
...
**Explicitly check every scaffolded `page.tsx` for branching logic** (redirects, multiple rendered states depending on data/auth/status) — not just its sub-components. This is easy to skip since `page.tsx` sits outside `_components/`. A thin pass-through page needs no test of its own; one with conditionals does. Confirmed gap on #420: a page rendered the wrong UI state for a failed payment with zero coverage, because only its sub-components had tests.
...
⚠️ **This check has been skipped in practice before, silently, with no error — same failure mode as the state-file write above. Treat it as a hard step: the critical-flow determination (yes or no) must appear as its own explicit line in the Breakpoint 6 summary below, every time, not just when the answer is yes.** (This has happened: #401 — a subscription/track access gate, unambiguously critical-flow — got no follow-up regression or e2e ticket filed, unlike #399's payment webhook, which got both (#550, #551).)
...
```bash
cd apps/academy && pnpm format:fix && pnpm lint && pnpm type-check && pnpm test
```
...
2. **Security — only if the diff touches a security area.** Check with `git diff --name-only origin/dev...HEAD`. It does if any file matches: `apps/academy/src/app/actions/**`, `apps/academy/src/app/api/**`, `apps/academy/src/app/auth/**`, `apps/academy/src/proxy.ts`, `apps/academy/src/lib/server/**`, `apps/academy/supabase/migrations/**`, `apps/academy/next.config.ts`, `apps/academy/.env.example`, `apps/academy/package.json`. If none match, state that security review was skipped and why, and go to step 3.
