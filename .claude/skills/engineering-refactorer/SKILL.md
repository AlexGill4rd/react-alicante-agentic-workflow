...
9. **Execute — Layout Composition** (components only):
   ...
   - Before extracting anything new, grep `components/atoms/`, `components/molecules/` and `components/organisms/` for an existing layout shell with the same recipe (e.g. border + radius + padding pill, icon + gap + text row). Reuse it instead of re-declaring the prop block.
