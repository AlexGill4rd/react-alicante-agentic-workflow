...
description: Bump the version in package.json to a given semver version and commit the change on the current release branch.
...
3. **Update version field** in `package.json` only — no other files.
4. **Commit:**
   ```bash
...
## Constraints
- Do NOT modify any file other than `package.json`.
