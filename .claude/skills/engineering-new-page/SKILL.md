...
## Prerequisites
- Confirm `src/app/[locale]/` exists.
- Confirm `src/app/[locale]/$ARGUMENTS/` does not already exist.
...
2. **Create `layout.tsx`:**
   ...
   - **Never import or wrap with `MainTemplate` here — that decision belongs in `page.tsx` only (Step 3).** `layout.tsx` can still render lightweight layout-level wrappers around `{children}` when the page needs them — e.g. `pricing/layout.tsx` renders `<ThemeSwitcher />` and `<PhilomathBreadcrumb />` around children. It's specifically the heavy site-chrome decision (header/footer/floating CTA via `MainTemplate`) that's reserved for `page.tsx`, not layout-level content in general.
3. **Create `page.tsx`:**
   - **Decide here whether this page needs full site chrome (header, footer, floating CTA) via `MainTemplate`, or should be a standalone, minimal-chrome screen.** Default to wrapping children in `MainTemplate` for ordinary content pages. Skip it for auth screens, error/unauthorized pages, or anything explicitly designed as a focused flow with its own back-navigation and no marketing chrome — matching the existing precedent (`ArticlesUnauthorizedPage`, `/login`). If unsure which category a new page falls into, check the Figma reference for visible header/footer chrome, or ask.
   - This decision lives in exactly one place. Don't let a later edit add `MainTemplate` back into `layout.tsx` or duplicate the wrapping in both files — that's how `/login` briefly regained the sitewide floating CTA after the layout had deliberately excluded it.
...
## Constraints
- No hardcoded strings — all visible text must use `next-intl` (`getTranslations` in Server Components, `useTranslations` only in Client Components that already need `'use client'` for other reasons).
...
- No importing `MainTemplate` in `layout.tsx` — that decision belongs in `page.tsx` only (see Step 3).
- No inline static data — static content belongs in `src/data/`.

## Output
- `src/app/[locale]/$ARGUMENTS/page.tsx`
- `src/app/[locale]/$ARGUMENTS/layout.tsx`
- `src/app/[locale]/$ARGUMENTS/_components/` directory with initial section component.
- Translation namespace added to `locales/en.json`.

## Verification
- [ ] `pnpm type-check` — zero new errors.
- [ ] `pnpm build` — catches Server/Client boundary errors (e.g. `createContext is not a function`) that `type-check` and Jest never execute against.
